...
# os outros itens
from models import ItemAgenda, UserProfile, Comentarios
from forms import FormItemAgenda, FormCreateUser, FormComentarios
# para o captcha
...
def lista(request):
	...
	try:
		...
		voteuser.up_voted = []
		voteuser.down_voted = []
		lista_itens = ItemAgenda.objects.filter()[:10]
...
def item_comentar(request, nr_item, nr_papai):
	if request.method == "POST":
		...
		if form.is_valid():
			...
			post = get_object_or_404(ItemAgenda, pk=nr_item) # contador de comentarios
